...
void convertMeshTo(Mesh& src, GCLF::Simplification::SMeshT& dst)
{
	...
	dst = GCLF::Simplification::SMeshT();
	for(auto vh : src.vertices())
	{
		OpenMesh::Vec3d& p = src.point(vh);
		GG::Vec3d v(p[0], p[1], p[2]);
		dst.add_vertex(v);
	}
	for(auto fh:src.faces())
	{
		auto fv_iter = src.fv_begin(fh);
		auto v0 = *fv_iter; fv_iter++;
		auto v1 = *fv_iter; fv_iter++;
		auto v2 = *fv_iter; fv_iter++;
		dst.add_face(v0,v1,v2);
	}
}
void convertMeshFrom(GCLF::Simplification::SMeshT& src, Mesh& dst)
{
	...
	for(auto vh : src.vertices())
	{
		GG::Vec3d& p=src.point(vh);
		OpenMesh::Vec3d v(p[0], p[1], p[2]);
		dst.add_vertex(v);
	}
	for(auto fh:src.faces())
	{
