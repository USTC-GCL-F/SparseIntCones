...
void output_middle_result(const SMeshT& mesh, const std::string& file_name, size_t cur_vn)
{
	// chech condition
	static size_t last_output_vn = 0;
	static size_t order = 0;

	if (cur_vn == last_output_vn)
		return;
	...
	SMeshT tmp = mesh;
	tmp.garbage_collection();
	last_output_vn = cur_vn;
	order++;
	OpenMesh::IO::write_mesh(tmp, file_name + "opt_" + std::to_string(order) + ".obj", OpenMesh::IO::Options::Default, 15);
}
