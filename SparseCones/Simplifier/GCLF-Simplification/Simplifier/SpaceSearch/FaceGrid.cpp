...
std::pair<std::pair<Vec3d, double>, int> FaceGrid::closest_point(const Vec3d& query)
{
  ...
  Vec3d result;
  Cell first, last, l;
  Sphere closest_sphere(query, closest_distance_sqr);
  ...
  if (is_in_ex(m_bbox, query))
  {
    ...
    for (l = first;l != last;++l) if (l->pidx >= 0)
    {
      ...
      BoundingBox& elem_box = m_prim_boxes[l->pidx];
      if (elem_box.do_intersect(closest_sphere))
      {
        ...
        if (distance_sqr < closest_distance_sqr)
        {
          ...
          closest_sphere.squared_radius = closest_distance_sqr;
        }
      }
    #else
      Vec3d cp = elem.tri.closest_point(query);
