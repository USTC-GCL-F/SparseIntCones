...
)
{
  ...
  f_check_self_inter = _f_check_self_inter;
}
...
bool EdgeFlipper::try_flip_edge()
{
  ...
  // check constraints
  if (f_check_wrinkle && flip_would_cause_wrinkle())
    return false;
  if (f_check_inter && flip_would_cause_intersection())
    return false;
  if (f_check_self_inter && flip_would_cause_self_intersection())
    return false;

  // do real flip and update
  return flip_edge();
}
...
bool EdgeFlipper::try_flip_edge_decrease_valence_avoid_out_of_bound(EdgeHandle e, double bound)
{
  if (!init(e))
    return false;
  ...
  if (local_Hausdorff_after_flipping() > bound)
    return false;
...
double EdgeFlipper::local_Hausdorff_after_flipping()const
{
  ...
  HeavyTriangle tri_a(pa1, pb0, pb1);
  HeavyTriangle tri_b(pa0, pa1, pb1);
...
#undef hd_for_links
  ...
  pre_calculate_edge_length(&local_rm);
  pre_calculate_face_area(&local_rm);

#ifdef USE_TREE_SEARCH
  ot->set_hint(pa0);
  hd = std::max(hd, calc_out_error(&local_rm, om, ot, infinite_fp));
  ot->unset_hint();
#else
  hd = std::max(hd, calc_out_error(&local_rm, om, og, infinite_fp));
#endif
  return hd;
}
...
bool EdgeFlipper::flip_would_cause_wrinkle()const
{
  auto& pa0 = rm->point(va0);
  auto& pa1 = rm->point(va1);
  auto& pb0 = rm->point(vb0);
  ...
  return (same_side_wrapped(pa1, pb1, pa0, pb0) >= 0
    || triangle_area(pa0, pa1, pb1) < GeomThr::area_de_thr
    || triangle_area(pb0, pa1, pb1) < GeomThr::area_de_thr);
}
...
bool EdgeFlipper::flip_would_cause_self_intersection()const
{
  auto& pa0 = rm->point(va0);
  auto& pa1 = rm->point(va1);
  auto& pb0 = rm->point(vb0);
  ...
  // 1. check overlap triangles with a common edge.
  {
  ...
  if (!rm->is_boundary(rm->opposite_halfedge_handle(edge))) {\
    ...
    if (triangle_do_overlap(p0, p1, p2, p_oppo_edge, ep0, ep1, ep2, ep_opp_edge))\
      return true;\
  }
  ...
  // 2. check intersect triangles with a common vertex.
  {
    { // (1) check faces have common vertex va1
      ...
      for (FaceHandle vf : rm->vf_range(va1))
      {
        ...
        if (vf != fa_ignore &&
          triangle_do_intersect(pb0, pb1, pa1, from_p, to_p, epb0, epb1, epa1, from_ep, to_ep))
          return true;

        if (vf != fb_ignore &&
          triangle_do_intersect(pb1, pa0, pa1, from_p, to_p, epb1, epa0, epa1, from_ep, to_ep))
          return true;
      }
    }
    {// (2) check faces have common vertex vb1
      ...
      for (FaceHandle vf : rm->vf_range(vb1))
      {
        ...
        if (vf != fa_ignore &&
          triangle_do_intersect(pa1, pb0, pb1, from_p, to_p, epa1, epb0, epb1, from_ep, to_ep))
          return true;

        if (vf != fb_ignore &&
          triangle_do_intersect(pa0, pa1, pb1, from_p, to_p, epa0, epa1, epb1, from_ep, to_ep))
          return true;
      }
    }
    {// (3) check faces have common vertex vb0
      ...
      for (FaceHandle f : check_faces)
      {
        ...
        ExactPoint* to_ep = rm->data(to_v).ep.get();
        ExactPoint* from_ep = rm->data(from_v).ep.get();
        if (triangle_do_intersect(pb1, pa1, pb0, from_p, to_p, epb1, epa1, epb0, from_ep, to_ep))
          return true;
      }
    }
    {// (4) check faces have common vertex va0
      ...
      for (FaceHandle f : check_faces)
      {
        ...
        ExactPoint* to_ep = rm->data(to_v).ep.get();
        ExactPoint* from_ep = rm->data(from_v).ep.get();
        if (triangle_do_intersect(pa1, pb1, pa0, from_p, to_p, epa1, epb1, epa0, from_ep, to_ep))
          return true;
      }
    }
  }
  // 3. check intersect non-adjacent triangles
  {
    //std::set<FaceHandle> one_ring_faces;
    ...
    // check triangle va1-vb0-vb1
    if (rt->do_intersect(pa1, pb0, pb1, epa1, epb0, epb1, _one_ring_faces))
      return true;
    // check triangle va0-va1-vb1
    if (rt->do_intersect(pa0, pa1, pb1, epa0, epa1, epb1, std::move(_one_ring_faces)))
      return true;
  }
  return false;
}

bool EdgeFlipper::flip_edge()
{
  // pre-update
  if (f_update_links)
    backup_in_links();
