...
{
	std::set<OpenMesh::EdgeHandle> edges_set;
	for (FaceHandle fh : faces)
		for (EdgeHandle feh : out_mesh->fe_range(fh))
			edges_set.insert(feh);
	std::vector<OpenMesh::EdgeHandle> edges(edges_set.begin(), edges_set.end());
	...
	distance = std::max(distance, calc_edge_out_error(out_mesh, in_mesh, in_tree, edges, threshold));
	if (distance >= threshold)	return DBL_MAX;
	// NOTE: won't calculate error for vertices
	return distance;
}
#else
...
{
	return same_side(a.x(), a.y(), a.z(), b.x(), b.y(), b.z(), c.x(), c.y(), c.z(), p.x(), p.y(), p.z());
}

/// @brief given two adjacent triangles (with a commen edge), check if they are coplanar (by an inexact approach).
/// @return 1 or -1: not coplanar, 0: coplanar.
int adjacent_triangles_coplanar_filtered(SMeshT* mesh, EdgeHandle eh)
{
	ASSERT(!mesh->is_boundary(eh), "boundary edge");
	HalfedgeHandle hh = mesh->halfedge_handle(eh, 0);
	CR_Vec3d a = mesh->point(mesh->from_vertex_handle(hh));
	CR_Vec3d b = mesh->point(mesh->to_vertex_handle(hh));
	CR_Vec3d c = mesh->point(mesh->opposite_vh(hh));
	CR_Vec3d d = mesh->point(mesh->opposite_he_opposite_vh(hh));
	return orient3d_filtered(a.x(), a.y(), a.z(), b.x(), b.y(), b.z(), c.x(), c.y(), c.z(), d.x(), d.y(), d.z());
}
	...
	CR_Vec3d new_point)
{
	for (HalfedgeHandle h : halfedges)
	{
		...
		double radian = angle_from_three_points(end_point, new_point, start_point);
		if (radian < GeomThr::radian_de_thr || M_PI - radian < GeomThr::radian_de_thr)
			return true;
...
)
{
	for (HalfedgeHandle h : halfedges)
	{
		...
		double length = (new_point - pp).norm();
